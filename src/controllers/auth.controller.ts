...
import { ForgotPasswordInput, LoginUserInput, RegisterUserInput, ResetPasswordInput, VerifyEmailInput } from '../schemas/user.schema'
import { createUser, findUniqueUser, findUser, signTokens, updateUser, deleteAll } from '../services/user.service'
...
export const registerUserHandler = async (req: Request<{}, {}, RegisterUserInput>, res: Response, next: NextFunction) => {
  try {
    ...
    const redirectUrl = `${config.get<string>('origin')}/api/auth/verifyemail/${verifyCode}`
    // const redirectUrl = `http://localhost:8000/api/auth/verifyemail/${verifyCode}`
    try {
...
export const loginUserHandler = async (req: Request<{}, {}, LoginUserInput>, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user.verified) {
      return next(new AppError(401, 'You are not verified, please verify your email to login'))
    }
...
export const verifyEmailHandler = async (req: Request<VerifyEmailInput>, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await updateUser({ verificationCode }, { verified: true, verificationCode: null }, { email: true })
...
export const forgotPasswordHandler = async (
  ...
  try {
    ...
    try {
      const url = `${config.get<string>('origin')}/api/auth/resetpassword/${resetToken}`
      ...
      await updateUser({ id: user.id }, { passwordResetToken: null, passwordResetAt: null }, {})
      return res.status(500).json({
        status: 'error',
...
export const deleteUsersHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    deleteAll()
    res.status(200).json({
      status: 'success',
